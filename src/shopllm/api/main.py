"""ShopLLM Gateway — v0.4.0: obs + redaction + guardrails + routing + auth + usage tracking."""
...
from shopllm.auth.deps import authenticate
from shopllm.db.models import Tenant, UsageEvent
...
async def chat_completions(
    ...
    if not await _rate_limiter.allow(tenant.id):
        log.warning("rate_limit.blocked", tenant_id=tenant.id)
    ...
    if redaction.total > 0:
        log.info("pii.redacted", tenant_id=tenant.id,
